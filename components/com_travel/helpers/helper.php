	 ...
	 $to =$_COOKIE['JBZooCurrencyToggle_current']; 
	  ...
	  $to ='gbp';
	    ...
	    if($to=='eur'){
		 ...
		 $symbol='C$';
		  $rate = $params->get('cad','1.26');
		  $ConvertedPrice =  round($pr * $rate);
		  $finalPrice =  $symbol . $ConvertedPrice;
		}else if($to=='gbp'){
		  $symbol='£';
		  $rate = $params->get('gbp','1');
		  $ConvertedPrice =  round($pr * $rate);
		  $finalPrice =  $symbol . $ConvertedPrice;
		}
		else{
...
static public  function  cancelPolicy( 
     $status, 
     $startDate = '',$endDate = '')

	 {
         if($status=='NonRefundable'){
			...
			$msg ='Room is fully non-refundable';
			 
		 }
		 if($status=='Refundable'){
			 
			$msg ='Room does not incur a cancellation charge at the time of booking';
			 
		 }
		 
		  if($status=='Unknown'){
			 $msg = 'Restrictions apply';
...
static public  function  convertPricevalue( 
     $pr, 
     $row = '', 
     $nac = true, 
     $nurpr = false) 
...
static public  function  currencycode() 
{
