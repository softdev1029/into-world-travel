...
if ($saveOrder){
	$saveOrderingUrl = 'index.php?option=com_yandex_maps&task=categories.saveOrderAjax';
	JHtml::_('sortablelist.sortable', 'categoryList', 'adminForm', $listDirn, $saveOrderingUrl);
}
?>
	  ...
	  <tbody>
		<?php foreach($this->items as $i=>$item){?>
			<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->map_id; ?>">
				...
				<td>
					<a class="btn btn-micro <?php echo $item->active ? 'active' : ''?>" href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i?>','categories.<?php echo $item->active?'unpublish':'publish'?>')"><i class="icon-<?php echo $item->active?'publish':'unpublish'?>"></i></a>
					<a class="btn btn-micro" title="<?php echo "Просмотр категрии на сайте"?>" target="_blank" href="<?php echo JRoute::_(JURI::root().'index.php?option=com_yandex_maps&task=category&id=' . (int) @$item->id)?>">
						<img style="height:14px;" src="<?php echo JURI::root().'/media/com_yandex_maps/images/view.png'?>">
					</a>
					<a class="btn btn-micro" title="<?php echo "Скопировать категорию"?>" target="_blank" href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i?>','categories.copy')"><img src="<?php echo JURI::root().'/media/com_yandex_maps/images/copy.png'?>"></a>
				</td>
				<td><a href="<?php echo JRoute::_('index.php?option=com_yandex_maps&task=categories.edit&id=' . (int) @$item->id)?>"><?php echo $item->title?></a></td>
				<td><a href="<?php echo JRoute::_('index.php?option=com_yandex_maps&task=objects&filter[mapid]='.((int)$item->map_id).'&filter[categoryid]=' . (int) @$item->id)?>"><?php echo $item->objects_count;?></a></td>
				<td><a href="<?php echo JRoute::_('index.php?option=com_yandex_maps&task=maps.edit&id=' . (int) @$item->map_id)?>"><?php echo $item->map_title?></a></td>
