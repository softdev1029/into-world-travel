...
$canOrder	= $user->authorise('core.edit.state', 'com_travel.otel');
$saveOrder	= 'a.ordering';
$saveOrder	= $listOrder == 'a.ordering';
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_travel&task=otels.saveOrderAjax&tmpl=component';
...
{
    if (task=='otels.import') return false;
...
<form action="<?php echo JRoute::_('index.php?option=com_travel&view=otels'); ?>" method="post" name="adminForm" id="adminForm">
...
<div id="j-main-container">
	 ...
	 <div class="btn-group pull-right hidden-phone">
         	...
         	<select name="filter_region" class="inputbox" onchange="this.form.submit()">
				<option value=""><?=JText::_("--Регион--")?></option>
				<?php echo JHtml::_("select.options",  FILTER_HTML::filter("region"), "value", "text", $this->state->get("filter.region"), true);?>
			</select>
...
<th width="20%" align="center" style='text-align:center'>
	   <?php echo JHtml::_('grid.sort', 'Регион', 'a.region', $listDirn, $listOrder); ?>
       </th>
	</tr>
</thead>
...
$linkEdit	= JRoute::_('index.php?option=com_travel&task=otel.edit&id='.$item->id);

$canCheckin	= $user->authorise('core.manage',		'com_checkin') || $item->checked_out==$user->get('id') || $item->checked_out==0;
$canChange	= $user->authorise('core.edit.state',	'com_travel.otel'.$item->id);
$canCreate	= $user->authorise('core.create',		'com_travel.otel'.$item->id);
?>
...
<td class="order nowrap center hidden-phone">
    ...
    border-radius: 10px;'>Наценка: ".$p."</span>";
	  ...
	  ?></center>
        ...
        <td><center>
	<?php  
		echo '<a href="'. JRoute::_($linkEdit).'">'. $this->escape($item->region_title).$item->region_title2.'</a>';
	  ?></center>
	</td>
...
onsubmit="return false;"  >
 ...
 <?php
  $db=JFactory::getDBO();
  $q = 'SELECT data FROM #__travel_hash WHERE t=1 ORDER BY data DESC';
  $db->setQuery($q);
  $row = $db->LoadObject();
  
 
   $q = 'SELECT COUNT(*) FROM #__travel_otel';
   $db->setQuery($q);
   $kol = $db->LoadResult();
 ...
 ?>
  ...
  <p style="line-height: 0.96;">
   ...
   В базе присутствует <strong style="<?=($kol) ? 'color:red' : ''?>"><?=$kol?></strong> отелей <br />
   <?php if ($row): ?>
...
<script type="text/javascript">
 ...
 //Вызвать окно 
  ...
  jQuery(document).on('click', '#subr', function () { 
  var obj = jQuery(this);
   start_import();
...
function start_import()
{
   ...
   jQuery.ajax({
   type: "GET",
   url:  '<?php echo $def."&task=import&start=1&hash=".$hash?>',
   data: m_data, 
...
function repeat_import() {var rand = getRandomInt(9999,99999999);
	jQuery.ajax({
			url: "<?php echo $def."&task=import&start=0&hash=".$hash?>&rand"+rand,
			timeout: 50000,
