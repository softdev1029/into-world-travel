...
jQuery.extend(jQuery.ui, { datepicker: { version: "1.8.2" } });
...
function Datepicker() {
	...
	this.dpDiv = jQuery('<div id="' + this._mainDivId + '" class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all ui-helper-hidden-accessible"></div>');
}
...
jQuery.extend(Datepicker.prototype, {
	...
	_doKeyDown: function(event) {
		...
		if (jQuery.datepicker._datepickerShowing)
			switch (event.keyCode) {
				...
				case 13: var sel = jQuery('td.' + jQuery.datepicker._dayOverClass, inst.dpDiv).
							add(jQuery('td.' + jQuery.datepicker._currentClass, inst.dpDiv));
						if (sel[0])
	...
	_generateHTML: function(inst) {
		...
		// вариант смены месяца и года работает только при текущем месяце с помощью - onclick="jQuery(\'.ui-datepicker-current-day\').trigger(\'click\');"
		// ] Yapro.Ru : lebnik
...
jQuery.datepicker.version = "1.8.2";
...
function datepickerYaproSetClockSelect(){// разбивает значение переменной YaproClock на чч.мм.сс и выставляет значения элементам SELECT
	if(typeof(YaproClock)=="string" && YaproClock!=""){
		...
		jQuery("select[name=h]").val(his[0]);
		jQuery("select[name=i]").val(his[1]);
		jQuery("select[name=s]").val(his[2]);
	}
}
function datepickerYaproSetClock(){// устанавливает значение переменной YaproClock и возвращает результат чч.мм.сс
	YaproClock = jQuery("select[name=h]").val()+":"+jQuery("select[name=i]").val()+":"+jQuery("select[name=s]").val();
	return YaproClock;
}
function datepickerYaproSetTime(){// выставляем значение элементу INPUT и скрываем календарь
	var e = jQuery(".datepickerYaproSelected");
	var date = new Date( jQuery(e).datepicker("getDate", "dd.mm.yy") );
	var Day    = date.getDate();
	var Month  = date.getMonth()+1;
	var Year   = date.getFullYear();
	if( Day < 10 ){ Day = "0" + Day; }
	if( Month < 10 ){ Month = "0" + Month; }
	jQuery(e).val(Year+"-"+Month+"-"+Day+" "+datepickerYaproSetClock());
	jQuery(e).datepicker("hide");// скрываем календарь
	//jQuery(e).removeClass("datepickerYaproSelected");// удаляем класс для потери возможности идентификации выбранного INPUT
}
jQuery(document).ready(function(){
	jQuery(".datepickerTimeField").datepicker({
		changeMonth: true,
		changeYear: true,
		dateFormat: 'yy-mm-dd',
		...
		jQuery(".datepickerYaproSelected").removeClass("datepickerYaproSelected");// удаляем со всех элементов класс идентификации
		jQuery(this).addClass("datepickerYaproSelected");// добавляем класс для возможности идентификации выбранного INPUT
