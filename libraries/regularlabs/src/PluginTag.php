...
class PluginTag
{
	 ...
	 * @return string
	 */
	public static function getRegexLeadingHtml($group = '')
	{
		$group = 'leading_block_element_' . $group;

		$block_elements = Html::getBlockElements(['div']);
		$block_element  = '(?P<' . $group . '>' . implode('|', $block_elements) . ')';

		$anything_but_tag = '(?:(?!</?(?P=' . $group . ')[ >]).)*?';
	 ...
	 * @return string
	 */
	public static function getRegexTrailingHtml($group = '')
	{
		$group = 'leading_block_element_' . $group;
