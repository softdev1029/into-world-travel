...
<a name="<?php echo $subcategory->alias; ?>"></a>
        ...
        <?php if ($vars['params']->get('template.subcategory_teaser_text', 1) && strlen($text) > 0) : ?>
            <div class="subcategory-description catalog__desc"><?php echo $text; ?></div>
        <?php endif; ?>


        <?php if (in_array($task, array('category', 'frontpage'))) : ?>
            <?php if ($maxItems != 0 && count($items) > 0) : ?>
                <div class="section-row subcategory-items clearfix">
                    <?php
                    foreach ($items as $item) {
                        echo '<div class="catalog__item">'.$this->app->jblayout->renderItem($item, 'subcategory_item').'</div>';
                    }
                    ?>
                </div>
            <?php endif; ?>
