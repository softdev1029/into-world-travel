...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="oba-content-layout">';
        $result .= '<div class="oba-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="oba-layout-cell' . ('oba-block' == $style ? ' oba-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="oba-post">' string in it.
     * Then it wraps the content of the buffer with the oba-post.
     */
    public function componentWrapper()
    {
        ...
        if (false === strpos($content, '<div class="oba-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'oba-main').addClass('oba-postcontent');
            }
        });
    }
});
